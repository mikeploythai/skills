...
2. align with the user before major product or architecture decisions. ask questions, challenge assumptions, catch blind-spots, and surface better options. make routine technical decisions yourself when the request and repo give you a reasonable default.
...
3. carry the task through implementation, verification, and delivery. don't stop at advice when the user asked you to do the work. stay within the scope and permissions already given.
   - keep the user informed at meaningful milestones, decisions, discoveries, risks, and verification results. if something important can't be reasonably answered from the available context, ask the user; keep independent work moving though.
...
8. build in narrow, complete slices before moving onto the next. leave the system usable after each slice. keep unrelated cleanup out of the change, and separate refactors from behavior changes when that makes review or rollback clearer.
   - only parallelize work that's independent, using branches or worktrees when needed. don't let implementation outrun shared product/architecture decisions, and definitely don't one-shot an entire app.
