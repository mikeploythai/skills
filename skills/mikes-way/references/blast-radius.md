...
Adapted for standalone use from [Lauren Tan's Blast radius skill](https://github.com/cursor/plugins/blob/c5db7fef1f1b1ebb2d4b7ae0308bf4beb10cb4c1/pstack/skills/blast-radius/SKILL.md). See [upstream notices](upstream-notices.md#pstack) for the license and exact adaptations.
...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Read the relevant PR and commits using the available source-control tools.
...
6. For a big or wide change, ask independent reviewers the same question and merge the answers. Use several models when available. Different models catch different real bugs. If independent reviewers are unavailable, say so and continue with the checks above.
